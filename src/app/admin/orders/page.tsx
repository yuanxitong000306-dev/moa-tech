      ...
      <main className="px-4 py-8 lg:px-8">
        <div className="mx-auto max-w-7xl">
          <div>
            ...
            <p className="mt-3 text-sm leading-6 text-gray-500">查看订单、会员信息和每个订单的商品明细。</p>
          </div>

          <section className="mt-8 grid gap-5">
            {orders.map((order) => (
              <article key={order.id} className="rounded-2xl border border-line bg-white p-5 shadow-sm">
                <div className="grid gap-5 xl:grid-cols-[260px_1fr_180px]">
                  ...
                  <div className="grid gap-3">
                    {(order.order_items ?? []).slice(0, 3).map((item, index) => (
                      <div key={`${item.product_id}-${index}`} className="flex items-center gap-3 rounded-xl bg-mist p-3">
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="mt-1 text-xs font-bold text-gray-500">
                            {item.product_brand} · {item.product_slug}
                          </p>
                          <p className="mt-1 text-xs text-gray-500">
                            {formatPrice(item.unit_price)} × {item.quantity} ={" "}
                            <span className="font-black text-ink">{formatPrice(item.subtotal_amount)}</span>
                          </p>
                        </div>
                      </div>
                    ))}
                    ...
                    {(order.order_items ?? []).length === 0 ? (
                      <p className="rounded-xl bg-mist p-4 text-sm font-bold text-gray-500">暂无商品明细</p>
                    ) : null}
                  </div>
                  ...
                  <div className="flex flex-col justify-between gap-4 xl:items-end">
                    ...
                    <Link href={`/admin/orders/${order.id}`} className="inline-flex h-10 items-center justify-center rounded-lg border border-line px-4 text-sm font-black text-gray-600 transition hover:border-ink hover:text-ink">
                      查看详情
                    </Link>
                  </div>
                </div>
            ...
            {orders.length === 0 ? (
              <div className="rounded-2xl border border-line bg-white p-10 text-center text-gray-500 shadow-sm">暂无订单</div>
            ) : null}
          </section>
        </div>
      </main>
