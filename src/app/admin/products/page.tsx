    ...
    <AdminShell>
      <main className="mx-auto max-w-7xl px-4 py-8 lg:px-8">
        ...
        <div className="mt-5 flex flex-col justify-between gap-4 md:flex-row md:items-end">
          ...
          <p className="rounded-md bg-white px-4 py-2 text-sm font-black text-gray-600 shadow-sm">共 {products.length} 件商品</p>
        </div>
        ...
        <section className="mt-8 grid gap-5">
          {products.map((product) => (
            <article key={product.id} className="rounded-lg border border-line bg-white p-5 shadow-sm">
              <div className="grid gap-5 lg:grid-cols-[140px_1fr_auto]">
                ...
                <form action={deleteProductAction}>
                  ...
                  <button className="rounded-md border border-red-200 px-4 py-2 text-sm font-black text-red-600 transition hover:bg-red-50">
                    删除
                  </button>
                </form>
              </div>
              <details className="mt-5 rounded-md bg-mist p-4">
